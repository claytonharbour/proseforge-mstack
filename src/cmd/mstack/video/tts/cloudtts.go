...
func NewCloudTTSCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "cloudtts [segments.json]",
		Short: "Generate TTS audio using Google Cloud Text-to-Speech API",
		Long:  "Generates TTS audio files (MP3) for each segment using Google Cloud Text-to-Speech API with OAuth authentication.",
	...
	cmd.Flags().BoolVar(&updateJSON, "update-json", false, "Update segments.json with .mp3 extension")
